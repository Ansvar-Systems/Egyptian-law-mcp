/**
 * get_egyptian_implementations — Find Egyptian statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetEgyptianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface EgyptianImplementationResult {
  document_id: string;
...
export async function getEgyptianImplementations(
  db: InstanceType<typeof Database>,
  input: GetEgyptianImplementationsInput,
): Promise<ToolResponse<EgyptianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as EgyptianImplementationResult[];
