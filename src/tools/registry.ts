/**
 * Tool registry for Egyptian Law MCP Server.
...
import { getEgyptianImplementations, type GetEgyptianImplementationsInput } from './get-egyptian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Al-Jarida Al-Rasmiyya (Official Gazette of Egypt). ' +
    ...
    'Call this FIRST when you need to understand what Egyptian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Egyptian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Egyptian statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Egyptian legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Egyptian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Egyptian?" ' +
  ...
  },
  {
    ...
    description:
      'Format an Egyptian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Egyptian statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get international legal instruments that a Egyptian statute references or aligns with. ' +
      'Egyptian laws may reference international standards and conventions ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Egyptian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_egyptian_implementations',
    description:
      'Find all Egyptian statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Egyptian statutes. ' +
      'Egyptian may reference international legal instruments where relevant.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Egyptian legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_egyptian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Egyptian legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Egyptian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for an Egyptian statute or provision. ' +
      ...
      'Egyptian is an EU Member State; this checks implementation status and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Egyptian statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_egyptian_implementations':
          result = await getEgyptianImplementations(db, args as unknown as GetEgyptianImplementationsInput);
          break;
