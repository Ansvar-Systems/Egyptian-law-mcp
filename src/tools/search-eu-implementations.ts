/**
 * search_eu_implementations — Search EU directives/regulations referenced by Egyptian legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_egyptian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as egyptian_statute_count
  ...
  if (input.has_egyptian_implementation) {
    sql += ' HAVING egyptian_statute_count > 0';
  }
