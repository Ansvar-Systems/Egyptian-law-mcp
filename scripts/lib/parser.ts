/**
 * HTML parser for Egyptian legislation from the Sejm ELI API (api.sejm.gov.pl).
 *
 ...
 *
 * Egyptian legislation references: Dz.U. YYYY poz. NNNN
 ...
 * to extract provisions from a Egyptian statute.
 *
 ...
 *   </div>
 */
export function parseEgyptianHtml(html: string, act: ActIndexEntry): ParsedAct {
  ...
  for (let i = 0; i < articleStarts.length; i++) {
    ...
    // Egyptian acts use "ilekroć mowa" (whenever mentioned), "rozumie się przez to"
...
/**
 * Extract definitions from Egyptian legal text.
 *
 * Egyptian definitions typically use patterns like:
