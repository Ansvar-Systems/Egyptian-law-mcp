#!/usr/bin/env tsx
/**
 * Egyptian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://manshurat.org';
const PORTAL_NAME = 'Manshurat (Egyptian Legislation)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/egyptian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Egyptian Law MCP — Data Freshness Check');
